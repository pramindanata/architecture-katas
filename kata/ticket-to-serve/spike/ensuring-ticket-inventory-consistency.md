...
When a transaction occurs (where it will decrement the stock number), I can apply the following queries (for example in PostgreSQL).
...
In PostgreSQL, update & delete statement will acquire a row level lock for the modified records.  It ensures others write queries that will write the same records need to wait until the lock is released. Technically the `select ... for update` is optional, it allows us to lock the row for longer period in case I need to do certain operations before writing the records.
...
This approach will ensure consistent decrement because each write will be treated like serial operation for each ticket record. Throughput will suffer, but it is the trade off that I need to make to solve this consistency problem.
